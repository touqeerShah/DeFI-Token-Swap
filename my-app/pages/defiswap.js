...
export default function Defiswap() {
  ...
  const [ConectionStatus, setConectionStatus] = useState("CONNECT");
  ...
  async function connect() {
    // this function will call and conec to metamask wallet
  ...
  async function displayBalance() {
    ...
    data.tokenBalances.find((item) => {
      console.log("item", item.tokenBalance.toString());
      let rawbalance = parseInt(item.tokenBalance.toString(), 16).toString();
      console.log("rawbalance", rawbalance);
      if (isNaN(rawbalance)) {
        rawbalance = "0".toString();
        console.log("isNaN ==>", rawbalance);
      }
      //   console.log(
      //     "Web3.utils.fromWei(rawbalance ==> ",
      //     Web3.utils.fromWei(rawbalance.toString())
      //   );
  ...
  async function getPrice() {
    ...
    //goerli.api.0x.org/swap/v1/quote?buyToken=${taddr}&sellToken=${sellToken}&sellAmount=${sellAmount}
    https: console.log("response == > ", response);
  ...
  return (
    <Grid.Container gap={1} justify="center">
      <Button
        ...
        color="primary"
        onPress={connect}
        css={{ boxShadow: "0px 0px 4px #000000", marginTop: "$15" }}
      >
        <Text
          css={{ color: "black" }}
          size={16}
      ...
      <Row justify="center">
        <Grid sm={4}>
          <Row>
            <Text size={20} css={{ marginLeft: "$5", color: "black" }}>
              Gas Estimate:{" "}
            </Text>
            <p
              style={{
          ...
          <Row>
            <Text size={24} css={{ marginLeft: "$5", color: "black" }}>
              LP Provider:{" "}
            </Text>
            <p
              style={{
          ...
          <Row justify="center">
            ...
            >
              <Row justify="center" css={{ mt: "$2" }}>
                ...
                <img src="alchemyblack.png" width={"30%"} />
              </Row>
              <Row justify="center" css={{ mt: "$2" }}>
                <Text
                  size={20}
